...
pub fn choose_random_move(state: &mut GameState, rng: &mut ChaCha12Rng) -> Option<ResolvedMove> {
    let moves = legal_resolved_moves(state);
    if moves.is_empty() {
        return None;
    }
...
pub fn choose_random_action_first(
    state: &mut GameState,
    rng: &mut ChaCha12Rng,
) -> Option<ResolvedMove> {
    let moves = legal_resolved_moves(state);
    ...
    let (non_pass, pass): (Vec<_>, Vec<_>) = moves
        .iter()
        .partition(|m| !matches!(m, ResolvedMove::Pass { .. }));
