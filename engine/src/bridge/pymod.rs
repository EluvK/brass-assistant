...
//!     .choose_heuristic_round() -> (first, second_opt, score)
...
impl PyGame {
    ...
    ///
    /// Full 2-action round planning entry point. Returns (first_canonical, second_canonical_opt, score).
...
/// Load a binary imitation shard generated by `gen_imitation`.
/// Returns list of tuples: (pid, era, value, abs_vp, winner, econ, snapshot, teacher_canonical).
#[pyfunction]
fn load_imitation_shard<'py>(
    py: Python<'py>,
    path: &str,
) -> PyResult<Vec<(usize, usize, [f32; 4], [f32; 4], [f32; 4], [f32; 2], Bound<'py, PyBytes>, String)>> {
    use std::fs::File;
    use std::io::BufReader;
    let file = File::open(path)
        .map_err(|e| pyo3::exceptions::PyIOError::new_err(format!("open {path}: {e}")))?;
    let mut reader = BufReader::new(file);
    let records: Vec<crate::ai::imitation::ImitationRecord> = bincode::deserialize_from(&mut reader)
        .map_err(|e| pyo3::exceptions::PyValueError::new_err(format!("deserialize {path}: {e}")))?;

    let mut out = Vec::with_capacity(records.len());
    for r in records {
        let py_bytes = PyBytes::new(py, &r.snapshot);
        out.push((
            r.pid,
            r.era,
            r.value,
...
/// Serialize records to a binary imitation shard (matching `gen_imitation` format).
#[pyfunction]
fn dump_imitation_shard(
    path: &str,
    records: Vec<(usize, usize, [f32; 4], [f32; 4], [f32; 4], [f32; 2], Vec<u8>, String)>,
) -> PyResult<()> {
    use std::fs::File;
    ...
    let to_save: Vec<crate::ai::imitation::ImitationRecord> = records
        .into_iter()
        .map(|(pid, era, value, abs_vp, winner, econ, snapshot, teacher_canonical)| {
            crate::ai::imitation::ImitationRecord {
                pid,
                era,
                value,
                abs_vp,
        ...
        .collect();
    bincode::serialize_into(&mut writer, &to_save)
        .map_err(|e| pyo3::exceptions::PyValueError::new_err(format!("serialize {path}: {e}")))?;
    Ok(())
}
...
fn _engine(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(load_imitation_shard, m)?)?;
    m.add_function(wrap_pyfunction!(dump_imitation_shard, m)?)?;
    m.add(
        "ACTION_FEATURE_DIM",
