...
### 3.2 回合行动（每回合必须选一个）
1. **Build 建厂** — 打出地点牌（在指定城市）或工业牌（在自己的网络内），消耗煤/铁建设建筑；运河时代每城市每玩家限 1 个板块
2. **Network 建网** — 打出地点牌，建造运河（时代1，£3）/ 铁路（时代2，£5+1煤；双铁路 £15+1煤+1啤酒）
3. **Develop 研发** — 消耗铁（1铁移除1个，最多2铁移除2个），移除面板上的低级建筑板块，解锁更高级；陶器 I/III 不可研发
4. **Sell 卖货** — 打出城市牌，需网络连通接受该货类的商家并支付啤酒，售出得钱，板块翻面计分
5. **Loan 贷款** — 贷款 £30，收入等级 **-3 级（不是 -10，收入可为负，下限 -10，回合末负收入需偿付）**
