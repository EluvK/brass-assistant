...
- **首轮每名玩家仅 1 个行动，其余轮次每人 2 个行动**
- **每次行动必须弃 1 张手牌**，行动后如果牌库仍有剩余，补满至 8 张
...
- **资源/连通性全部走 GameState 缓存，废除 BFS 搜图（2026-08 落地）**：`state.rs` 维护 `free_coal_mines`/`free_iron_works`/`free_beer_cubes`（`place_tile`/`remove_tile`/`consume_*`/`auto_sell_to_market`/`end_canal_era` 的 `rebuild_free_sources()`/`resolve_shortfall` 的 `remove_city_tile_by_key` 维护；啤酒缓存还覆盖 `farm_tiles`，`sync_farm_beer`/`drop_farm_beer` 单独维护，debug 下 `apply_move` 末尾 `assert_caches_consistent` 自检）+ `component_cache` 连通分量位掩码（39 链接存在性指纹惰性重建，对测试/双铁路 dry-run 的直接 `links` 写入自动自愈；`RwLock` 因 PyO3 需 Send+Sync）。`graph::find_coal_sources`/`find_iron_sources`/`connected_locations`/`find_beer_sources` 都只读缓存，**无 BFS**；`find_beer_sources` 输出顺序 = 旧全扫顺序（自家桶按 key/农场序、对手桶按连通位置升序），保证 replay 字节一致。新增成本函数 `graph::coal_purchase_cost(state, loc, needed) -> Option<i32>` / `iron_purchase_cost(needed) -> i32`（免费先抵扣→市场按槽位价逐桶最便宜优先→General Supply 空市价 8/6）。`coal_price()`/`iron_price()` 仅作"市场紧张度"标量信号（NN 特征 g[42]/g[43] 与启发式），**不是**多桶成本估算器
...
- **合法动作生成是单一来源（2026-08 落地）**：`rules.rs` 只有一个 `generate_moves(state, MoveExpansion)`（`All` = raw 全枚举，`OnePerSlot` = 每 policy 槽一个代表），`legal_moves`/`legal_slot_moves` 只是薄包装——**不要再加第二条生成器**，否则与 MCTS/策略头覆盖必然漂移。双铁路 dry-run 与执行全部走 `RailTx` 事务守卫（Drop 保证任意返回/panic 回滚），勿改回手工回滚
- **游戏循环统一走 `game_loop::play`（2026-08 落地）**：main/replay/stat_game/sweep_*/bench_mcts/debug_mcts/sweep_mcts 全部用共享驱动（`choose` 返回 `Option<Move>`，hooks：`before_move`/`after_move`/`on_era`→`AfterEra`/`after_era`）。新增全局面回放/扫描二进制时**直接用 `game_loop::play`，勿手写循环**；多 hook 需共享同一份统计时用 `RefCell` 绕开闭包 `&mut` 别名冲突
