...
class Sample:
    ...
    abs_vp: np.ndarray | float = 0.0  # (4,) absolute score `(vp - 100) / 50`, absolute seat order
...
def play_game_with_roles(
    ...
    abs_vp = ((np.asarray(vps, dtype=np.float32) - 100.0) / 50.0)
...
def _generate_imitation_game(args):
    ...
    abs_vp = ((vps - 100.0) / 50.0).astype(np.float32)
    ...
    for pid, era, snapshot, canon in steps:
        ...
        local.append(Sample(
            pid=pid, era=era, value=value, abs_vp=abs_vp, winner=winner,
...
def load_bin_shard(path: str | os.PathLike) -> list[Sample]:
    """Load a binary imitation shard generated by Rust's `gen_imitation`."""
    records = be.load_imitation_shard(str(path))
    samples: list[Sample] = []
    for pid, era, val, abs_vp, win, econ, snap_bytes, canon in records:
        samples.append(Sample(
            pid=pid,
            era=era,
            value=np.asarray(val, dtype=np.float32),
            abs_vp=np.asarray(abs_vp, dtype=np.float32),
            winner=np.asarray(win, dtype=np.float32),
            econ=np.asarray(econ, dtype=np.float32),
            snapshot=bytes(snap_bytes),
            teacher_canonical=canon,
        ))
    return samples
